...
(define-key  lfg-mode-map "\C-c\C-x" 'run-new-xle)
...
(cond ((not xle-mode-map)
       ...
       (define-key xle-mode-map [menu-bar xle close-all]
                '("Close all XLE windows" . close-all))
...
(define-key  xle-mode-map "\C-c\C-a" 'close-all)
(define-key  xle-mode-map "\C-c\C-p" 'xle-create-parser)
...
(defun close-all ()
      (interactive)
       (send-string (current-buffer) "close-all")
...
(defun lfg-skip-comments ()
   ...
   (cond ((looking-at "\"")
          (progn (forward-char 1)
                 ...
                 (skip-chars-forward " \t\n")
                 (lfg-skip-comments)))
         ((looking-at "\<[Cc][Oo][Mm][Mm][Ee][Nn][Tt]\>")
          (progn (forward-char 1)
                 (re-search-forward "\<\/[Cc][Oo][Mm][Mm][Ee][Nn][Tt]\>" nil t)
                 (skip-chars-forward " \t\n")
                 (lfg-skip-comments)))
         (t)))
...
(defun lfg-in-comment (bufferpos config-pos)
  "Use sexp parser to determine whether we are in or out of a comment 
   beginning and ending with double quotes, which parse-partial-sexp 
   recognizes as a string.  NOT DONE: recognizing whether we are in a comment
   surrounded by <comment>...<\comment>.  If we do this, we will slow things
   down a lot."
  (save-match-data
     (save-excursion
     (goto-char config-pos)
